...
const Analyst1StatsPage = () => {
  ...
  const fetchStats = async () => {
    try {
      setLoading(true)
      const credits = await getCreditsAnalyst1('')
      setStats({
        total: credits.length,
        submitted: credits.filter((c: CreditData) => c.status === 'SUBMITTED').length,
        inReview: credits.filter((c: CreditData) => c.status === 'ANALYST1_REVIEW').length,
        returned: credits.filter((c: CreditData) => c.status === 'ANALYST1_RETURNED').length,
        approved: credits.filter((c: CreditData) => c.status === 'ANALYST1_APPROVED').length,
  ...
  return (
    <div className='space-y-8'>
      <div>
        ...
        <p className='text-gray-600 mt-1'>Métricas de rendimiento y procesamiento</p>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <div className='bg-white rounded-2xl shadow-md p-6 border border-gray-100'>
          <div className='flex items-center justify-between mb-4'>
            <h3 className='text-lg font-semibold text-gray-700'>Total Procesados</h3>
            <span className='text-3xl'>📊</span>
          </div>
          <p className='text-4xl font-bold text-blue-600'>{stats.total}</p>
          <p className='text-sm text-gray-500 mt-2'>Créditos asignados</p>
        </div>
        ...
        <div className='bg-white rounded-2xl shadow-md p-6 border border-gray-100'>
          ...
          <p className='text-4xl font-bold text-yellow-600'>{stats.submitted + stats.inReview}</p>
      ...
      <div className='bg-white rounded-2xl shadow-md p-6 border border-gray-100'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-5 gap-4'>
          <div className='text-center p-4 bg-green-50 rounded-xl'>
            ...
            <p className='text-sm text-green-600 mt-1'>Radicados</p>
          </div>
          ...
          <div className='text-center p-4 bg-blue-50 rounded-xl'>
            <p className='text-3xl font-bold text-blue-700'>{stats.approved}</p>
            <p className='text-sm text-blue-600 mt-1'>Aprobados</p>
          </div>
        ...
        <div className='space-y-4'>
          ...
          <div>
            <div className='flex justify-between mb-2'>
              ...
              <span className='text-sm font-medium text-yellow-600'>{stats.submitted + stats.inReview}</span>
            </div>
            <div className='w-full bg-gray-200 rounded-full h-3'>
              <div
                className='bg-yellow-500 h-3 rounded-full transition-all'
                style={{ width: `${stats.total > 0 ? ((stats.submitted + stats.inReview) / stats.total) * 100 : 0}%` }}
              />
            </div>
          </div>
          <div>
...
export default Analyst1StatsPage
