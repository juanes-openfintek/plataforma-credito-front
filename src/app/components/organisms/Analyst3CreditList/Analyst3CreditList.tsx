...
const Analyst3CreditList = ({ creditsList, page, perPage, onSelectCredit }: Props) => {
        ...
        <table className='w-full'>
          ...
          <tbody>
            {paginatedCredits.map((credit, index) => {
              ...
              return (
                <tr key={credit._id || index} className='border-b hover:bg-gray-50'>
                  <td className='p-3'>{credit.radicationNumber || credit.code}</td>
                  <td className='p-3'>{credit.name} {credit.lastname}</td>
                  <td className='p-3'>${formatCurrency(credit.amount)}</td>
                  <td className='p-3'>
                    <TagStatus text={statusProp?.text || credit.status} background={statusProp?.background} />
                  </td>
                  <td className='p-3'>
                    {new Date(credit.radicationDate || credit.created).toLocaleDateString()}
                  </td>
                  <td className='p-3 text-center'>
                    <button
                      onClick={() => onSelectCredit(credit)}
                      className='bg-primary-color text-white px-4 py-2 rounded-lg hover:bg-opacity-90'
                    >
                      {credit.status === 'PENDING_SIGNATURE' ? 'Ver' : 'Revisar'}
                    </button>
                  </td>
                </tr>
              )
...
export default Analyst3CreditList
